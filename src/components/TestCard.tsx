...
export function TestCard({ test, onSelect, index }: TestCardProps): JSX.Element {
  ...
  return (
    <motion.div
      ...
      className="group relative bg-white rounded-3xl p-8 shadow-sm hover:shadow-xl hover:shadow-pink-100/50 transition-all duration-300 border border-neutral-100 hover:border-pink-100 flex flex-col h-full"
    >
      <div className="mb-6">
        <span className="inline-block px-3 py-1 rounded-full bg-pink-50 text-pink-500 text-xs font-bold uppercase tracking-wider">
          {test.category}
        </span>
      </div>
